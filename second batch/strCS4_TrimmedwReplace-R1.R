...
setting=strsplit(setting[[1]],',')
  tryCatch({
    ...
    for (t in 1:nsim)
    {
      ...
      per=0.05*n1
      rightdelete=rank(loglik)[(n1-per):n1]
      leftdelete=rank(loglik)[1:per]
      #delete=c(rank(loglik)[(n1-19):n1])
      
      a[leftdelete,]=a[rank(loglik)[per+1],]
      a[leftdelete,]=a[rank(loglik)[n1-per-1],]
      
      n1=nrow(a)
      amu=t(matrix(apply(a,2,sum)/(n1)))
      delta1=matrix(amu,q,p)
      
      data1=matrix(0,n1,p*q)
      ...
      if(p==2){
        for(i in 1:q)
        {
          ...
          data1[,(1+(i-1)*p):(i*p)]=a[,c(i,(i+q))]   #measurement 1
      ...
      if(p==3){ 
        for(i in 1:q)
        {
          ...
          data1[,(1+(i-1)*p):(i*p)]=a[,c(i,(i+q),(i+2*q))]   #measurement 1
          gp1bar[,(1+(i-1)*p):(i*p)]=t(matrix(apply(data1[,(1+(i-1)*p):(i*p)],2,sum)/n))
          gp1a=sweep(data1[,(1+(i-1)*p):(i*p)],MARGIN=2,gp1bar[,(1+(i-1)*p):(i*p)],FUN="-")
          gp1ass=apply(gp1a^2,2,sum)
          gp1an=sweep(gp1a,MARGIN=2,sqrt(gp1ass),FUN="/")
      ...
      delta1=matrix(apply(t(matrix(amu,q,p)),2,sum)/p,q,1)
        ...
        
      {
        ...
        mlsige=mlsig
        imlsig=solve(mlsig)
        k5=tr((diag(p)%x%imlsig)%*%f1)
        l5=tr(((one%*%t(one))%x%imlsig)%*%f1)
        ...
        # rr=nk3/(-2*n*s*q)
        k01=n1*q*(p-1)*p
        pp=(k01-s*k01+k5*s^2-s*l5)/(s*k01)
        qq=(2*s*k5-k01)/(s*k01)
        rr=(k5-l5)/(s*k01)
        aa=(1/3)*(3*qq-pp^2)
      ...
      per=0.05*n2
      rightdelete=rank(loglik)[(n2-per):n2]
      leftdelete=rank(loglik)[1:per]
      #delete=c(rank(loglik)[(n1-19):n1])
      
      b[leftdelete,]=b[rank(loglik)[per+1],]
      b[leftdelete,]=b[rank(loglik)[n2-per-1],]
      n2=nrow(b)
      bmu=t(matrix(apply(b,2,sum)/(n2)))
      delta2=matrix(apply(t(matrix(bmu,q,p)),2,sum)/p,q,1)
      w2=matrix(0,p*q,p*q)
      for (i in 1:n2)
      {
        subb=b[i,]
      ...
      if(p==2){
        for(i in 1:q)
        {
          ...
          data1[,(1+(i-1)*p):(i*p)]=b[,c(i,(i+q))]   #measurement 1
          gp1bar[,(1+(i-1)*p):(i*p)]=t(matrix(apply(data1[,(1+(i-1)*p):(i*p)],2,sum)/n))
          gp1a=sweep(data1[,(1+(i-1)*p):(i*p)],MARGIN=2,gp1bar[,(1+(i-1)*p):(i*p)],FUN="-")
          gp1ass=apply(gp1a^2,2,sum)
          gp1an=sweep(gp1a,MARGIN=2,sqrt(gp1ass),FUN="/")
      ...
      if(p==3){ 
        for(i in 1:q)
        {
          ...
          data1[,(1+(i-1)*p):(i*p)]=b[,c(i,(i+q),(i+2*q))]   #measurement 1
          gp1bar[,(1+(i-1)*p):(i*p)]=t(matrix(apply(data1[,(1+(i-1)*p):(i*p)],2,sum)/n))
          gp1a=sweep(data1[,(1+(i-1)*p):(i*p)],MARGIN=2,gp1bar[,(1+(i-1)*p):(i*p)],FUN="-")
          gp1ass=apply(gp1a^2,2,sum)
          gp1an=sweep(gp1a,MARGIN=2,sqrt(gp1ass),FUN="/")
      ...
      while ((maxab>converge)&(iter<100))
      {
        ...
        k6=tr((diag(p)%x%imlsig)%*%f2)
        l6=tr(((one%*%t(one))%x%imlsig)%*%f2)
        #kk3=tr((diag(p)%x%imlsig)%*%w)
        #mk3=tr((c1%x%imlsig)%*%w)
        #nk3=tr((c2%x%imlsig)%*%w)
        ####solving the cubic equation ####
        ...
        # rr=nk3/(-2*n*s*q)
        k02=n2*q*(p-1)*p
        pp=(k02-s*k02+k6*s^2-s*l6)/(s*k02)
        qq=(2*s*k6-k02)/(s*k02)
        rr=(k6-l6)/(s*k02)
        aa=(1/3)*(3*qq-pp^2)
        bb=(1/27)*(2*pp^3 -9*pp*qq +27*rr)
    ...
    file=paste ("str_CS4_trimmedreplacement_Row#",as.numeric(setting[[1]][11]), ".csv", sep="")
