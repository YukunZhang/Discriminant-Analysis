  ...
  tryCatch({
    ...
    for (t in 1:nsim)
    {
      ...
      while ((maxab>converge)&(iter<100))
      {
        ...
        s=p-1
        pp=(k7)/(-2*n1*s*q)
        qq=(2*n1*s*q-2*k5-2*k6)/(-2*n1*s*q)
        rr=k7/(-2*n1*s*q)
      ...
      while ((maxab>converge)&(iter<100))
      {
        ...
        s=p-1
        pp=(k7)/(-2*n2*s*q)
        qq=(2*n2*s*q-2*k5-2*k6)/(-2*n2*s*q)
        rr=k7/(-2*n2*s*q)
   ...
   out[1]=setting[1]
    ...
    file=paste ("Ustr_AR4_trimmedreplacement_Row#",as.numeric(setting[[1]][11]), ".csv", sep="")
